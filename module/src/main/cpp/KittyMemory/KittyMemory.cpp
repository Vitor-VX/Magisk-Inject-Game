...
uintptr_t
KittyMemory::getRealOffeset(uintptr_t offset, KittyMemory::ProcMap libAddress) {
    uintptr_t baseAddress = reinterpret_cast<uintptr_t>(libAddress.startAddr);

    uintptr_t sumOffsets = baseAddress + offset;
    return sumOffsets;
}
...
uintptr_t
KittyMemory::getAbsoluteAddress(const char *libraryName, unsigned int relativeAddr, bool useCache) {
    ProcMap libMap;
